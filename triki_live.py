...
class LiveGestureDetector:
    ...
    def add_sample(
        ...
        if prediction.confidence < self.min_confidence:
            self._clear_candidate()
            return None
        if not self._is_active_now(prediction.label, elapsed_seconds):
            self._clear_candidate()
            return None
        if not self._confirm_candidate(prediction.label):
            return None
        if prediction.label in self._one_shot_latches:
            return None
        if not self._should_emit(prediction.label, elapsed_seconds):
            return None
