...
The repo is a flat set of `triki_*.py` modules — no package install step. Full build and platform setup is in [docs/build.md](docs/build.md).
...
- **The motion engine** (`triki_motion_engine.py`) — cleaner separation of the five controls, less ghosting, on a wider range of hands and surfaces. If you change tuning, validate against recorded sessions, not just by feel.
...
- Don't reintroduce a "directional joystick" without solving the heading problem ([docs/how-it-works.md](docs/how-it-works.md)) — it can't be faked in software on this hardware.
