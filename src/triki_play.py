...
from triki_battery import BATTERY_LEVEL_UUID, battery_snapshot, normalize_battery_percent
from triki_key_emitter import DEFAULT_KEYMAP, KeyOutputController, NullKeyEmitter
...
# Control payloads (gesture mappings) are small JSON; bound the request body so a
...
# Live cap visualization streams raw axes at ~15 Hz so the WebView can animate
# the real cap PNG and show a 6-axis readout. This is purely additive: it does
# NOT touch sample parsing, gesture detection, classifier thresholds, or BLE
# timing -- it only builds a small dict and puts it on the existing event bus.
MOTION_PUBLISH_INTERVAL_SECONDS = 1.0 / 15.0


def build_motion_event(
    values: Sequence[int], elapsed_seconds: float
) -> dict:
    """Build an additive 'motion' SSE payload from a sample's 6 raw axes.

    values = (a, b, c, d, e, f) where a,b,c = gyro and d,e,f = accel
    (see triki_protocol.MotionSample). The frontend normalizes these itself;
    we add only two cheap derived hints: ``energy`` (gyro vector magnitude, a
    motion-intensity scalar for glow/pulse) and ``rotation`` (gyro-c, the
    cap-spin axis = values[2]). No thresholds or tuning are applied here.
    """
    axes = [int(v) for v in values]
    a, b, c = axes[0], axes[1], axes[2]
    energy = (a * a + b * b + c * c) ** 0.5
    return {
        "type": "motion",
...
async def run_ble_session(
    ...
    *,
    ...
    def on_notify(_sender, data: bytearray) -> None:
        nonlocal event_count, last_sample_publish, last_motion_publish
        elapsed = time.monotonic() - started_at
        for sample in parser.feed(bytes(data)):
            sample_payload = session.record_sample()
            event = detector.add_sample(elapsed, sample)
            # Additive live-cap stream: throttled independently of the 0.25s
            # sample/state gate so the cap animation stays smooth (~15 Hz).
            # Non-blocking: just build a small dict and publish.
            if elapsed - last_motion_publish >= MOTION_PUBLISH_INTERVAL_SECONDS:
                bus.publish(build_motion_event(sample.values, elapsed))
...
<body>
  ...
  <script>
    let state = null;
    const keyChoices = ['', 'left', 'right', 'up', 'down', 'enter', 'space', 'escape', 'tab', 'backspace', 'page-up', 'page-down', '=', 'ctrl', 'shift', 'alt', 'w', 'a', 's', 'd', 'volume-up', 'volume-down', 'volume-mute', 'media-play-pause', 'media-next', 'media-prev'];
